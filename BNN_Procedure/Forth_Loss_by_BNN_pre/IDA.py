...
def IDA_1record(FEModel:mops.MDOFOpenSees, IM_list:list, EQRecordfile:str, period:float, 
    DeltaT = 'AsInRecord'):
    # Parameters:
    #   EQRecordfile:  earthquake record file. no file extension .unit: g  地震记录文件名
    #   IM_list: unit: g  强度指标(Intensity Measure)列表
    #   period: fundamental period 结构基本周期
    #   DeltaT: time step of dynamic analyses （动力分析的时间步长）

    FEModel.UniqueRecorderPrefix = 'URP'+ Path(EQRecordfile).name +'_'

    IDA_result = pd.DataFrame()  #创建空DataFrame用于存储IDA分析结果
